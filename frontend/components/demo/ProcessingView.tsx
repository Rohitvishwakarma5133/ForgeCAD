...
import { openSSE, fetchReport } from '@/lib/backend';
import { CheckCircle, Loader2, Upload, Brain, Search, GitBranch, ShieldCheck, AlertCircle } from 'lucide-react';
import { ProcessingStatus } from '@/types';

function normalizeStageName(input?: string, progress?: number): 'upload' | 'preprocessing' | 'symbol_detection' | 'text_extraction' | 'connection_tracing' | 'validation' | 'complete' {
  const s = (input || '').toLowerCase();
  if (s.includes('complete')) return 'complete';
  if (s.includes('file intake') || s.includes('upload')) return 'upload';
  if (s.includes('ocr')) return 'text_extraction';
  if (s.includes('ai analysis') || s.includes('analysis')) return 'symbol_detection';
  if (s.includes('data storage') || s.includes('storage')) return 'validation';
  if (s.includes('report') || s.includes('builder')) return 'validation';
  if (s.includes('qa') || s.includes('validation')) return 'validation';
  if (s.includes('connection') || s.includes('trace')) return 'connection_tracing';
  if (s.includes('preprocess')) return 'preprocessing';
  // Fallback by progress ranges
  if (typeof progress === 'number') {
    if (progress < 15) return 'upload';
    if (progress < 35) return 'preprocessing';
    if (progress < 55) return 'symbol_detection';
    if (progress < 75) return 'text_extraction';
    if (progress < 90) return 'connection_tracing';
...
export default function ProcessingView({ filename, conversionId, onComplete, onError }: ProcessingViewProps) {
  ...
  const pollConversionStatus = useCallback(async () => {
    ...
    try {
      const report = await fetchReport(conversionId);
      const status = report.status;
      if (status === 'done') {
        const json = report.aiJson || {};
        const items = Array.isArray(json.items) ? json.items : [];
        const transformedResult = {
          conversionId: report._id,
          filename: report.filename || filename,
          equipmentCount: items.length || 0,
          pipeCount: 0,
          instrumentCount: 0,
          confidence: 0.9,
          processingTime: 0,
      ...
      if (status === 'error') {
        onError?.(report.error || 'Processing failed');
        return;
      }
      // still processing
  ...
  useEffect(() => {
    ...
    try {
      es = openSSE(conversionId);

      const handleStage = (stage?: string) => {
        const normalized = normalizeStageName(stage);
        const s = processingStages.find(x => x.stage === normalized) || processingStages[0];
        setCurrentStageData(s);
        setCurrentStage(processingStages.indexOf(s));
      };
      ...
      es.addEventListener('complete', async (ev: MessageEvent) => {
        try {
          const data = JSON.parse((ev as any).data || '{}');
          const reportId = data.reportId || conversionId;
          ...
          onError?.('Failed to fetch report after completion');
          try { es?.close(); } catch {}
        }
      });

...
class EnhancedTagParser {
     ...
     */
    static processTag(originalTag, dwgReferenceTag) {
        const appliedRules = [];
        ...
        if (beforeNormalization !== normalizedTag) {
            if (/[—_]/.test(beforeNormalization))
                appliedRules.push('hyphen_normalization');
            if (/(?<=\d)O|O(?=\d)/.test(beforeNormalization))
                appliedRules.push('O_to_0_substitution');
            if (/(?<=\D)I(?=\d)|(?<=\d)I(?=\D)/.test(beforeNormalization))
                appliedRules.push('I_to_1_substitution');
            if (/\s/.test(beforeNormalization))
                appliedRules.push('space_removal');
            if (beforeNormalization !== beforeNormalization.toUpperCase())
                appliedRules.push('case_normalization');
        }
        ...
        if (validation.isValid) {
            ...
            if (validation.criticality === 'CRITICAL')
                confidence += 0.1;
            else if (validation.criticality === 'HIGH')
                confidence += 0.05;
        }
     ...
     */
    static batchProcess(extractedTags, dwgReferenceTags) {
        const results = [];
        ...
        const characterConfusionCount = results.filter(r => r.appliedRules.some(rule => rule.includes('substitution'))).length;
     ...
     */
    static generateValidationReport(results, stats) {
     ...
     */
    static validateCriticalTags(tags) {
        ...
        const foundCritical = tags.filter(tag => criticalPrefixes.some(prefix => tag.startsWith(prefix)));
...
exports.EnhancedTagParser = EnhancedTagParser;
// Production regex patterns based on specification
EnhancedTagParser.TAG_PATTERNS = {
...
EnhancedTagParser.MASTER_REGEX = new RegExp(Object.values(EnhancedTagParser.TAG_PATTERNS)
    .map(pattern => `(${pattern.source})`)
    .join('|'));
exports.default = EnhancedTagParser;
